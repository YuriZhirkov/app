...
export default {
  ...
  mounted() {
    // 根据是否传递值来将一系列进行默认值赋值
    this.smsCls =
      this.$options.propsData.cls === undefined
        ? "code-sms"
        : this.$options.propsData.cls;
    this.smsBtn =
      this.$options.propsData.btn === undefined
        ? "点击发送"
        : this.$options.propsData.btn;
    this.smsNum =
      this.$options.propsData.time === undefined
        ? 60
        : this.$options.propsData.time;
    this.deNum =
      this.$options.propsData.time === undefined
        ? 60
        : this.$options.propsData.time;
